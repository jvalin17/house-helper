...
class TrackerService:
    ...
    def create(
        ...
        app_id = self._repo.create_application(
            job_id=job_id,
            resume_id=resume_id,
            cover_letter_id=cover_letter_id,
        )
        return self._repo.get_application(app_id)

    def update_status(self, app_id: int, status: str) -> dict:
        self._repo.update_status(app_id, status)
        return self._repo.get_application(app_id)

    def list_applications(self, status: str | None = None) -> list[dict]:
        return self._repo.list_applications(status=status)

    def get_application(self, app_id: int) -> dict | None:
        return self._repo.get_application(app_id)

    def get_status_history(self, app_id: int) -> list[dict]:
        return self._repo.get_status_history(app_id)
